<template>
  <div class="page-content">
    ...
    <el-dialog v-model="dialogVisible" :title="isEdit ? '编辑上机物料' : (isCopy ? '复制上机物料' : '新增上机物料')" width="900px" :close-on-click-modal="false" destroy-on-close>
...
const searchForm = reactive({ keyword: '', factory: '', isOutOfWarranty: '' })
const dateRange = ref([])
...
const defaultForm = {
  ...
  repairPerson: '', confirmer: '', repairRequestTime: '', startTime: '',
...
function handleReset() {
  Object.assign(searchForm, { keyword: '', factory: '', isOutOfWarranty: '' })
...
async function handleExport() {
  try {
    ...
    downloadBlob(response.data, '上机物料.xlsx')
